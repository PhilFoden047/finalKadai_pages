...
interface SolarSystemConfig {
  ...
  earthRadius: number;
  moonRadius: number;
  earthOrbitPeriod: number; // 秒
  moonOrbitPeriod: number;  // 秒
}
...
class SolarSystemApp {
  ...
  private earthOrbitGroup!: THREE.Group;
  private moonOrbitGroup!: THREE.Group;
  
  // 軌跡関連
  private earthTrail: THREE.Vector3[] = [];
  private moonTrail: THREE.Vector3[] = [];
  private earthTrailLine!: THREE.Line;
  private moonTrailLine!: THREE.Line;
  ...
  // 速度設定
  private earthOrbitSpeedMultiplier: number = 1;
  private moonOrbitSpeedMultiplier: number = 1;
  private rotationSpeedMultiplier: number = 1;
  
  // カメラビュー
  private currentViewIndex: number = 0;
  private readonly viewPositions: ViewPosition[] = [
  ...
  // 設定
  private readonly config: SolarSystemConfig = {
    earthOrbitRadius: 25,
    moonOrbitRadius: 6,
    sunRadius: 5,
    earthRadius: 2,
  ...
  // 不要なメソッドを削除
  // private updateSpeedDisplay(), toggleAnimation(), resetAnimation(), switchView() は削除
  ...
  private updateInfoDisplay(elapsed: number, earthOrbitSpeed: number, moonOrbitSpeed: number): void {
